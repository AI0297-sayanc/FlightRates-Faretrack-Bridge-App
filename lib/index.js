...
const Schedule = require("../models/flightrates/Schedule")
...
module.exports = {
  ...
  async flightRatesApiRollback(shopIds, _user) {
    if (shopIds.length === 0) return
    try {
      const deletedBy = { what: "Customer", _who: _user, when: new Date() }
      await Promise.all([
        /* eslint-disable max-len */
        Schedule.updateMany({ _shop: { $in: shopIds }, _user }, { deletedBy, isDeleted: true }),
        // Hook.updateMany({ _shop: req.params.id, _user: _user }, { deletedBy, isDeleted: true }),
        Shop.updateOne({ _id: { $in: shopIds }, _user }, { deletedBy, isDeleted: true })
        /* eslint-enable max-len */
      ])
      // eslint-disable-next-line consistent-return
      return logger.info("Sucessfully rollback ✅!!")
    } catch (error) {
      // eslint-disable-next-line consistent-return
